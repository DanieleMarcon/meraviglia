...
- proposal-document roundtrip import payload families must pass through explicit runtime decode/adaptation seams (with version-aware dispatch where applicable) before re-entering downstream orchestration, and decoder seams must remain structural/compatibility-only (no business normalization).
...
- bootstrap decode/adaptation seams that emit migration metadata (for example `shouldWriteBackCanonicalEnvelope`) must keep writeback decisions in bootstrap orchestration and only perform opportunistic canonical persistence when decoded payloads are valid, preserving fail-closed behavior for invalid/unsupported inputs
...
- repository/infra egress adapters that write to external systems must pass repository input payloads through explicit runtime adaptation seams before SDK/database calls (shape guard -> canonical external write payload), instead of forwarding structural-trust objects directly
- decode/adaptation adapters at infra boundaries may perform structural/compatibility normalization only (shape guards, legacy key aliases, nullability coercion) and must not perform business/domain normalization
